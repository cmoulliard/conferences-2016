...
auth_result=$(curl -k -X POST $host/auth/realms/demo/protocol/openid-connect/token -d grant_type=password -d username=admin -d password=admin -d client_id=demo)
access_token=$(echo -e "$auth_result" | awk -F"," '{print $1}' | awk -F":" '{print $2}' | sed s/\"//g | tr -d ' ')

echo $access_token

url=https://apiman-gateway.vagrant.f8/gateway/demo/cdi-cxf/4.0
